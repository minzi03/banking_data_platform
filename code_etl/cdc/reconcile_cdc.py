...
def main() -> int:
    ...
    # `--mode` is part of the CLI contract but only the Spark path is
    # implemented. Parsed so callers passing it do not fail; not read yet.
    ...
    try:
        for name, (source, topic, bronze, expected) in EXPECTED.items():
            ...
            if (
                not spark.sql("SHOW TABLES IN lakehouse.bronze")
                .filter(F.col("tableName") == bronze.split(".")[-1])
                .take(1)
            ):
