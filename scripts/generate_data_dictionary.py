...
def render(tables: list[Table]) -> str:
    ...
    a("")
    a("- **Sẽ bỏ sót** cột nhạy cảm đặt tên không theo mẫu thông dụng. Đã đo:")
    a("  mẫu không có `cccd` — số định danh cá nhân, và là trường nhạy cảm nhất")
    a("  của nền tảng — nên nó KHÔNG được đánh dấu ở cả 5 bảng chứa nó. Các cột")
    a("  khác cũng bị bỏ sót: `device_id`, `account_no`, `ip_address`,")
    a("  `latitude`/`longitude`, `manager_name`.")
    ...
    a("")
    a("Vì giới hạn thứ nhất, **đừng đọc con số trên là số cột PII**. Bản kiểm kê")
    a("đầy đủ, dựng bằng cách parse DDL với danh sách cột rộng hơn, là")
    a("[`PII_INVENTORY.md`](../06-security-compliance/PII_INVENTORY.md) §2.")
    a("")
