...
# (chuỗi bị cấm, lý do)
BANNED_CLAIMS = [
    ("18 Gold analytics tables", "8 CTAS *_current đã retire; còn 10 historical Gold + 9 dbt serving"),
    ("18 Analytics Tables", "cùng lý do — đếm cũ gộp cả CTAS đã retire"),
    ("12 dbt models", "12 model sm_* là passthrough ephemeral và đã bị gỡ; còn 9 serving model"),
    (
    ...
    ("persisted per-partition watermarks", "cùng lý do"),
    ("using per-partition watermarks", "cùng lý do"),
    (
    ...
    ("completed in under one minute", "cùng lý do — không trial nào của v1.1 dưới 1 phút"),
    (
    ...
    ("Gold Analytics<br/>18 Tables", "node mermaid vẫn đếm cả 8 bảng CTAS đã retire"),
    ("dbt<br/>12 Gold Models", "dbt không còn 12 semantic model; nó publish 9 serving model"),
...
class TestDocsAgreeOnArchitecture:
    ...
    def test_all_docs_state_serving_ownership(self):
        ...
        """
        missing = [p.name for p in DOCS if p.exists() and "serving" not in p.read_text(encoding="utf-8").lower()]
        assert not missing, f"tài liệu không nhắc tầng serving: {missing}"

    def test_all_docs_state_time_semantics(self):
        missing = [p.name for p in DOCS if p.exists() and "from_utc_timestamp" not in p.read_text(encoding="utf-8")]
        assert not missing, (
    ...
    def test_mermaid_serving_node_exists_where_a_diagram_exists(self):
        ...
        for path in DOCS:
            ...
            if "```mermaid" not in text:
                continue
            assert "iceberg.serving" in text, f"{path.name}: sơ đồ không có node tầng serving do dbt sở hữu"
